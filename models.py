# for generating the CNN implementing YOLO and extracting feature vectors
...
###############################################################################
############# Handpicked layers from the convolutional network ################
frame_nos = [8, 23, 28, 32, 49, 52, 65, 72 , 74, 77, 81, 92, 96, 99, 114, 115] 
###############################################################################
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, var=0):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ##########################################
            ## Made changes here to bring out layers
            if i == 100:
                a = x[0,frame_nos,:,:]
                a = a.flatten()
                a = a.cpu()
        ...
        #### And here ..... 
        return sum(output) if is_training else (torch.cat(output, 1), a)
        ##################################################################
