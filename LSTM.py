...
class bboxLSTM(nn.Module):
    def __init__(self, path_cfg):
        ...
        layer_list = parse_model_cfg(path_cfg)
        
        self.linear1 = nn.linear(in_features = layer_list[0]['in_features'], out_features = layer_list[0]['out_features'])
        self.linear2 = nn.linear(in_features = layer_list[1]['in_features'], out_features = layer_list[1]['out_features'])
        self.LSTM = nn.LSTM(input_size = layer_list[2]['input_size'], hidden_size = layer_list[2]['hidden_size'], num_layers = layer_list[2]['num_layers']) 
        self.linear3 = nn.linear(in_features = layer_list[3]['in_features'], out_features = layer_list[3]['out_features'])
